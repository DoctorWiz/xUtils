...
namespace Vamperizer
{
	public partial class frmTune : Form
	{
		...
		private const string helpPage = "http://wizlights.com/xUtils/Vamperizer";
		private string applicationName = "Vamperizer";
		private string thisEXE = "Vamperizer.exe";
		...
		private void InitForm()
		{
			...
			//string mySubDir = "W⚡zsterSoftware\\xUtils\\" + appFolder + "\\";
			string mySubDir = "WizsterSoftware\\xUtils\\" + appFolder + "\\";
		...
		private void GetTheControlsFromTheHeartOfTheSun()
		{
			...
			SetCombo(cboStepSize,							Properties.Settings.Default.stepSize);
			...
			SetCombo(cboMethodSegments,				Properties.Settings.Default.methodSegments);
			SetCombo(cboLabelsSegments,				Properties.Settings.Default.labelSegments);
		...
		private void SetTheControlsForTheHeartOfTheSun()
		{
			...
			Properties.Settings.Default.stepSize					= cboStepSize.Text;
			...
			Properties.Settings.Default.methodSegments		= cboMethodSegments.Text;
			Properties.Settings.Default.alignSegments			= cboAlignSegments.Text;
			Properties.Settings.Default.labelSegments			= cboLabelsSegments.Text;
		...
		private void ConfirmAnnotator()
		{
			...
			if (!System.IO.File.Exists(annotatorProgram))
			{
				...
				if (setForm.closeMode == DialogResult.OK)
				{
					...
					{
						string msg = "Vamperizer cannot continue until the Queen Mary Vamp Plugin has been installed.";
				...
				{
					string msg = "Vamperizer cannot continue until Sonic Annotator and the Queen Mary Vamp Plugin have been installed.";
		...
		private void ExportSelectedTimings(string fileName)
		{
			...
			if (doBarsBeats)
			{
				if (xBars != null)
				{
					...
					{
						WriteTimingFileX(xBars, fileName);
						WriteTimingFile4(xBars, fileName);
						WriteTimingFile5(xBars, fileName);
						writeCount+=3;
					}
				}
			}
			if (chkBeatsFull.Checked)
			{
				if (xBeatsFull != null)
				{
					...
					{
						WriteTimingFileX(xBeatsFull, fileName);
						WriteTimingFile4(xBeatsFull, fileName);
						WriteTimingFile5(xBeatsFull, fileName);
						writeCount+=3;
					}
				}
			}
			if (chkBeatsHalf.Checked)
			{
				if (xBeatsHalf != null)
				{
					...
					{
						WriteTimingFileX(xBeatsHalf, fileName);
						WriteTimingFile4(xBeatsHalf, fileName);
						WriteTimingFile5(xBeatsHalf, fileName);
						writeCount+=3;
					}
				}
			}
			if (chkBeatsThird.Checked)
			{
				if (xBeatsThird != null)
				{
					...
					{
						WriteTimingFileX(xBeatsThird, fileName);
						WriteTimingFile4(xBeatsThird, fileName);
						WriteTimingFile5(xBeatsThird, fileName);
						writeCount+=3;
					}
				}
			}
			if (chkBeatsQuarter.Checked)
			{
				if (xBeatsQuarter != null)
				{
					...
					{
						WriteTimingFileX(xBeatsQuarter, fileName);
						WriteTimingFile4(xBeatsQuarter, fileName);
						WriteTimingFile5(xBeatsQuarter, fileName);
						writeCount+=3;
					}
				}
			}
			if (chkNoteOnsets.Checked)
			{
				if (xOnsets != null)
				{
					...
					{
						WriteTimingFileX(xOnsets, fileName);
						WriteTimingFile4(xOnsets, fileName);
						WriteTimingFile5(xOnsets, fileName);
						writeCount+=3;
					}
				}
			}
			if (chkTranscribe.Checked)
			{
				if (xTranscription != null)
				{
					...
					{
						WriteTimingFileX(xTranscription, fileName);
						WriteTimingFile4(xTranscription, fileName);
						WriteTimingFile5(xTranscription, fileName);
						writeCount+=3;
					}
				}
			}
			if (chkPitchKey.Checked)
			{
				if (xKey != null)
				{
					...
					{
						WriteTimingFileX(xKey, fileName);
						WriteTimingFile4(xKey, fileName);
						WriteTimingFile5(xKey, fileName);
						writeCount += 3;
					}
				}
			}
			if (chkSegments.Checked)
			{
				if (xSegments != null)
				{
					...
					{
						WriteTimingFileX(xSegments, fileName);
						WriteTimingFile4(xSegments, fileName);
						WriteTimingFile5(xSegments, fileName);
						writeCount += 3;
					}
				}
			}
		...
		err = SafeCopy(timingsTemp, expFile);
